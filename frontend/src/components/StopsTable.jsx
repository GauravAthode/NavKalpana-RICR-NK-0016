...
export default function StopsTable({ planResult }) {
  ...
  return (
    <GlassCard>
      <div className="text-sm font-semibold text-brand-light">Charging Stops</div>
      ...
      <div className="mt-4 overflow-x-auto">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {stops.length === 0 ? (
              <tr>
                <td colSpan={8} className="py-6 text-brand-light/60">
                  No charging stops required for this trip.
                </td>
              </tr>
            ) : (
              stops.map((s, i) => (
                <tr key={i} className="border-b border-brand-light/20">
